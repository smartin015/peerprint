...
func TestOneHop(t *testing.T) {
  ...
  if cont := c.Step(context.Background(), 100); !cont {
    t.Errorf("Expected c.Step=true, got false")
  }
  if want := 1; got != want {
...
func TestMultiHop(t *testing.T) {
  ...
  c:= NewCrawl([]peer.AddrInfo{aa[0]}, peerChain)
  for i := 0; i < 4; i++ {
    if cont := c.Step(context.Background(), 100); cont != true {
  ...
  if cont := c.Step(context.Background(), 100); cont != false {
    t.Errorf("got cont=%v, want false", cont)
  }
...
func TestFanOutBatched(t *testing.T) {
  ...
  c:= NewCrawl([]peer.AddrInfo{a1}, peerChain)
  ...
  if cont := c.Step(context.Background(), 100); cont != true {
    t.Errorf("Want cont = true") // We haven't crawled the new peers yet
  }
